...
constexpr float GyroMeasError = PI * (40.0f / 180.0f);   // gyroscope measurement error in rads/s (start at 40 deg/s)
constexpr float GyroMeasDrift = PI * (0.0f  / 180.0f);   // gyroscope measurement drift in rad/s/s (start at 0.0 deg/s/s)
...
#define SQRT_34 0.86602540378f //sqrtf(3.0f / 4.0f)
constexpr float beta = SQRT_34 * GyroMeasError;   // compute beta
constexpr float zeta = SQRT_34 * GyroMeasDrift;   // compute zeta, the other free parameter in the Madgwick scheme usually set to a small or zero value
constexpr float Kp = 2.0f * 5.0f; // these are the free parameters in the Mahony filter and fusion scheme, Kp for proportional feedback, Ki for integral
...
// quaternion-based estimate of absolute device orientation -- which can be converted
// to yaw, pitch, and roll. Useful for stabilizing quadcopters, etc.
...
extern "C" void MadgwickQuaternionUpdate()
{
...
extern "C" void MahonyQuaternionUpdate()
{
    float q1 = q[0], q2 = q[1], q3 = q[2], q4 = q[3];   // short name local variable for readability
    float norm;
    float hx, hy, bx, bz;
